...
class FEP_Admin_Settings
{
	...
	public function admin_menu()
	{
		...
		$my_page = add_submenu_page(
			'elementor',
			__('Flexible Editor Panel', 'flexible-editor-panel'),
			__('FEP Settings', 'flexible-editor-panel'),
			'manage_options',
			'fep-options',
			[$this, 'plugin_page']
	...
	{
		$mylinks = array(
			'settings' => '<a href="' . admin_url('admin.php?page=fep-options') . '">' . __('Settings', 'flexible-editor-panel') . '</a>',
		);
	...
	private function fep_admin_sections()
	{
		...
		$sections = [
			...
			[
				...
				'id'     => 'fep_informations',
				'title'  => esc_html__('Informations', 'flexible-editor-panel'),
				'submit' => false,
			...
			[
				...
				'title'  => esc_html__('How to configure', 'flexible-editor-panel'),
				'submit' => false,
			...
			[
				...
				'id'     => 'fep_debug',
				'title'  => esc_html__('Debug', 'flexible-editor-panel'),
				'submit' => false,
	...
	public function plugin_page()
	{
		...
		$html_fep_title .= '<h1>' . __('Flexible Editor Panel', 'flexible-editor-panel') . '</h1>';
		$html_fep_title .= '<h3>' . __('Makes the Elementor editor panel flexible, draggable, resizable, foldable and more.', 'flexible-editor-panel') . '</h3>';
		$html_fep_title .= '</div>';
	...
	public function save_message()
	{
		...
		if (isset($_GET['settings-updated'])) { ?>
			...
			<div class="updated notice is-dismissible">
				...
				<p><strong><?php esc_html_e('Your settings have been saved.', 'flexible-editor-panel') ?></strong></p>
	...
	public function footer_info()
	{
		...
		<div class="fep-options-footer-info">
			...
			<p><?php //_e('footer text', 'flexible-editor-panel');
				?></p>
...
<?php
	...
	public function enqueue_fep_admin()
	{
		...
		// style
		wp_enqueue_style('fep-admin', FEP_URL . '/admin/assets/css/settings.css', array(), FEP_VERSION, 'all');

		// scripts
		wp_enqueue_script('fep-functions-admin', FEP_URL . '/admin/assets/js/functions-admin.js', false, FEP_VERSION, true);
		wp_localize_script(
			'fep-functions-admin',
			'ajax_var',
			array(
		...
		wp_enqueue_script('fep-admin', FEP_URL . '/admin/assets/js/settings.js', false, FEP_VERSION, true);
		wp_localize_script(
			'fep-admin',
			'text_var',
			array(
				'confirm' => __('Do you confirm that action?', 'flexible-editor-panel')
			)
		);
	}
}
